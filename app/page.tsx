...
const icons=[Sparkles,Atom,Atom,FlaskConical,Layers3,Layers3,Lightbulb,FlaskConical,BookOpen];
export function Classroom(){
 ...
 <Tabs value={chapter} onValueChange={v=>go(String(v))} orientation="vertical" className="course-shell"><aside className={`course-nav ${menu?'nav-open':''}`}><div className="nav-heading">S3 <span>化學課程</span></div><TabsList className="chapter-list" aria-label="課程單元">{chapters.map(([id,zh,en],i)=>{const Icon=icons[i];return <TabsTrigger key={id} value={id} className="chapter-item"><span className="chapter-icon"><Icon size={19}/></span><span><b>{zh}</b><small>{en}</small></span>{visited.includes(id)&&id!==chapter?<Check size={13} className="visited"/>:<span className="chapter-num">{String(i).padStart(2,'0')}</span>}</TabsTrigger>})}</TabsList><div className="nav-note"><BookOpen size={19}/><p>由實物到小粒子<br/><b>一步一步，睇明化學。</b></p></div><div className="progress-note">已瀏覽 {visited.length} / {chapters.length} 個單元<div className="reading-progress"><span style={{width:`${visited.length/chapters.length*100}%`}}/></div></div></aside>
 <main tabIndex={-1} id="lesson-main" className="lesson-main"><div className="breadcrumb"><span>中三化學</span><span>/</span><b>{chapters[index][1]}</b><span className="unit-count">{String(index).padStart(2,'0')} / 08</span></div>
 ...
 <TabsContent value="elements"><Elements/></TabsContent><TabsContent value="atom"><AtomStructure teacher={teacher}/></TabsContent><TabsContent value="formula"><FormulaGallery key={teacher?'teacher':'student'} teacher={teacher}/></TabsContent><TabsContent value="classify"><Classification teacher={teacher}/></TabsContent><TabsContent value="compare"><Comparison/></TabsContent><TabsContent value="changes"><Changes/></TabsContent><TabsContent value="lab"><Laboratory/></TabsContent><TabsContent value="review"><Review teacher={teacher}/></TabsContent>
