...
export default function PeriodicTable(){
 ...
 const isFirstTwenty=(atomicNumber:number)=>atomicNumber<=20;
 return <section className="periodic-section"><div className="section-label"><div><span className="eyebrow">PERIODIC TABLE</span><h2>{en?'A map of all the elements':'元素週期表：元素的地圖'}</h2></div><span>{en?'118 elements · first 20 highlighted':'118 個元素 · 頭 20 個已突出'}</span></div><p className="periodic-intro">{en?'One box, one element. Select it to explore.':'一格一種元素，點選查看。'}</p><div className="periodic-toolbar"><label><Switch checked={highlight} onCheckedChange={setHighlight} aria-label={en?'Highlight the first 20 elements':'突出頭 20 個元素'}/><span>{en?'Highlight the first 20 elements':'突出頭 20 個元素'}</span></label><div className="periodic-legend">{Object.entries(categories).map(([id,label])=><span key={id}><i className={id}/>{label}</span>)}</div></div><div className="periodic-scroll" tabIndex={0} role="region" aria-label={en?'Periodic table, scroll sideways on smaller screens':'元素週期表，小螢幕可左右捲動'}><div className={`periodic-grid ${highlight?'highlight-first20':''}`}>
