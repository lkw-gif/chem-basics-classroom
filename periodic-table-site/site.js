import { shellElectronPositions, modelMarkup, createClassroomModel } from './models-3d.js';
...
function atomDiagram(symbol, arrangement) {
  ...
  const circles = arrangement.map((count, shell) => {
    ...
    const dots = shellElectronPositions(count, radius, center).map(({ x, y }) =>
      `<circle class="electron-dot" cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="5" />`
    ).join('');
...
function setupGroupOne() {
  const root = document.querySelector('#water-model');
  if (!root) return;
  const model = createClassroomModel(root, 'Li', en);
  const select = (symbol) => {
    ...
    model.setElement(symbol);
    document.querySelector('#water-copy').innerHTML = `<span class="eyebrow">${symbol} · ${t(item.zh, item.en)}</span><h3>${t('播放，觀察變化', 'Play and observe')}</h3><p>${t(item.noteZh, item.noteEn)}</p><div class="model-takeaway"><b>H₂ · ${t('氫氣', 'Hydrogen')}</b><p>${t('金屬逐漸消失，留下鹼性溶液。', 'The metal disappears, leaving an alkaline solution.')}</p></div><p class="model-prompt">${t('比較 Li、Na、K：哪個反應最快？', 'Compare Li, Na and K: which reacts fastest?')}</p>`;
  };
...
function setupGroupTwo() {
  const root = document.querySelector('#acid-model');
  if (!root) return;
  const model = createClassroomModel(root, 'Mg', en);
  const select = (symbol) => {
    ...
    model.setElement(symbol);
    document.querySelector('#acid-copy').innerHTML = `<span class="eyebrow">${symbol} · ${t(item.zh, item.en)}</span><h3>${t('稀鹽酸中的變化', 'In dilute hydrochloric acid')}</h3><p>${t(item.noteZh, item.noteEn)}</p><div class="model-takeaway"><b>${symbol === 'C' ? t('碳作比較', 'Carbon for comparison') : `H₂ · ${t('氫氣', 'Hydrogen')}`}</b><p>${symbol === 'C' ? t('碳仍然留在試管中，沒有明顯氣泡。', 'Carbon remains in the tube with no obvious bubbles.') : t('金屬逐漸溶解，氣泡上升。', 'The metal dissolves as bubbles rise.')}</p></div><p class="model-prompt">${t('比較 Mg 和 Ca：哪個反應較快？', 'Compare Mg and Ca: which reacts faster?')}</p>`;
  };
...
function setupHalogens() {
  ...
  if (!detail) return;
  const model = createClassroomModel(document.querySelector('#halogen-model'), 'Cl', en);
  const select = (symbol) => {
    model.setElement(symbol);
    ...
    detail.innerHTML = `<div><span class="eyebrow">${symbol}₂ · ${t('雙原子分子', 'Diatomic molecule')}</span><h3>${t(item.zh, item.en)} <small>${en ? symbol : item.en}</small></h3><p>${t('室溫狀態：', 'State at room temperature: ')}<b>${t(item.stateZh, item.stateEn)}</b></p><p>${t('實際外觀：', 'Sample colour: ')}<b>${t(item.colourZh, item.colourEn)}</b></p><div class="model-takeaway"><b>${t('兩球相連＝一個分子', 'Two joined spheres = one molecule')}</b><p>${['F', 'Cl'].includes(symbol) ? t('分子相隔較遠，向不同方向移動。', 'Molecules are far apart and move in different directions.') : symbol === 'Br' ? t('分子靠近，可以互相滑過。', 'Molecules stay close and slide past one another.') : t('分子排列整齊，在原位振動。', 'Molecules form an ordered arrangement and vibrate in place.')}</p></div>${symbol === 'I' ? `<p>${t('碘受熱昇華，可形成紫色蒸氣。', 'Iodine can sublime on heating to form purple vapour.')}</p>` : ''}</div>`;
...
function setupNoble() {
  ...
  if (!display) return;
  const model = createClassroomModel(document.querySelector('#noble-model'), 'Ne', en);
  const select = (symbol) => {
    model.setElement(symbol);
